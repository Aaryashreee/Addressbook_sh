...
list_items()
{
  ...
  if [ "$#" -eq "0" ]; then
    echo -e "Enter name to be searched : "
    read search
    if [ -z "$search" ]; then
      search="."
    fi
  ...
  echo -en "Matches found in the file: "
  num_lines "$search"
}

add_item()
{
  echo "Add Item: You will be adding NAME , PHONE , EMAIL :"
  echo -e "Name: "
  read name
  find_lines "^${name}:"
  if [ `num_lines "^${name}:"` -ne "0" ]; then
    echo "Sorry, $name already has been entered."
    echo "Try adding new Name"
    return
  fi
  echo -e "Phone: "
  read phone
...
locate_single_item()
{
  echo -en "Item to searched in the file: "
  read search
  n=`num_lines "$search"`
  if [ -z "$n" ]; then
    n=0
...
remove_item()
{
  ...
  list_items "$search"
  confirm " You want to $search to be removed ?"
  if [ "$?" -eq "0" ]; then
